...
For each surviving candidate $P$, let $X_i$ denote the ratio of the candidate's value for metric $i$ to the corresponding standard value, where $i \in \{\text{size}, \text{kg}, \text{sg}, \text{sv}, \text{cb}\}$. Working with ratios rather than raw values keeps the five axes on a common, dimensionless scale. The Euclidean distance from the origin to $P$ is then
\begin{equation}
    d(P) = \sqrt{X_{\text{size}}^2 + X_{\text{kg}}^2 + X_{\text{sg}}^2 + X_{\text{sv}}^2 + X_{\text{cb}}^2}\,,
...
As before, each $X_i$ is the ratio of the candidate's metric to its standard counterpart, so the five axes remain on a common dimensionless scale.
