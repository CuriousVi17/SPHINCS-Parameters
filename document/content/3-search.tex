...
Someone can say it's pretty good. Our opinion is that 12\% win on the signature size isn't very attractive, we want to reduce it at least by 20\%. Of course, in the cost of something else...

\subsubsection{Case 3: Adaptive Search} 
The strategy is very simple:
\begin{enumerate}
    \item Determine what sacrifices you are willing to make for each parameter.
    \item Find the best between all remaining candidates.
\end{enumerate}

For (1) we can apply filters we introduced in Sec. 3.3. This section is dedicated to (2).

We can define each candidate by a tuple of metrics value.  So, the search for optimal candidate could be represented as finding the closest point to the origin in five dimension space (similar to the finding the shortest vector, fortunately within very limited number of candidates), where each dimension represents a particular metric. That's almost it! 

Let's assume we have only 2 metrics (signature size and signature generation cost) we care about. We want to keep the signature size $\leq$ SLH-DSA, and we are fine if the signature generation will take at most 2$\times$ from std.  In this case, indeed, we can allocate all candidates on 2-dimension space. All candidates are allocated in green square (what simplifies their brute-force). And definitely the best candidate is the closes point to (0,0).

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{content/images/scatter_plot.png}
    \caption{Visualization of Candidates with Prioritization of Size and Signing Cost Metrics.}
    \label{fig:scatter_plot}
\end{figure}


We couldn't visualize 5-dimensional space, but we came with idea how to show it differently. 

First of all we plot unbounded parameters set (with only condition to have 128 bits security) on the plane randomly (the position doesn't matter, each candidate is defined by unique set of parameters). After initializing candidates, we apply the signature size filter, visualized as color filtering card. Moving further apply all other filter unil the last one.

The resulting set is the set with left candidates where we can search for the best. 
